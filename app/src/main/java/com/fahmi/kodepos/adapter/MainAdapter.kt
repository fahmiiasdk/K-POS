package com.fahmi.kodepos.adapter
...
import com.fahmi.kodepos.R
import com.fahmi.kodepos.model.ModelMain
import com.fahmi.kodepos.realm.RealmHelper
...
internal class MainAdapter(private val context: Context, private val modelMain: MutableList<ModelMain>) :
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.imageFavorite.setOnFavoriteChangeListener { buttonView, favorite ->
            if (favorite) {
                ...
                helper?.addFavorite(Provinsi.toString(),
                    Kabupaten.toString(), Kecamatan.toString(), Kelurahan.toString(), KodePos.toString())
                Snackbar.make(buttonView, data.strKelurahan + " Added to Favorite", Snackbar.LENGTH_SHORT).show()
            } else {
                helper?.deleteKodePos(data.strKelurahan.toString())
